...
def dpo_loss_fn(model, config, data, dropout_rng, params, reference_params, is_train=True):
  ...
  loss = -jax.nn.log_sigmoid(ratios) * (1 - LABEL_SMOOTHING) - jax.nn.log_sigmoid(-ratios) * LABEL_SMOOTHING
  loss = jnp.mean(loss, -1)
  valid_mask = (
      (data["chosen_segmentation"] != 0) & (data["rejected_segmentation"] != 0) & (data["chosen"] != data["rejected"])
  )
